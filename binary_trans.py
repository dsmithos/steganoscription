...
# rowHisto = list(bin_image.projection_rows())
...
# Im.fromarray(image_data).crop((10,minima_list[i-1][0],width,(minima_list[i][0]))).save("test_segment_"+str(i)+".png")

bin_image.save_image(sys.argv[2])

# def imgCrop(im):
#     box = (20, height-20, 20, width-20)
#     region = im.crop(box)
#     im.save(sys.argv[2])

# im = Im.open(sys.argv[2])
# imgCrop(im)

# px = bin_image.get((10,10))
# py = bin_image.get((width-10,height-10))
# vRect = Rect(Point(10,10),Point(width-10,height-10))
# hRect = Rect((0,10),(width-10,height-10))
# cropped_img = imgCrop(bin_image)
# new_image = Im.fromarray(image_data_new)
# cropped_img.save_image(sys.argv[2])
##########################################################
